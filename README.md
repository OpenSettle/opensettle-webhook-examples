...
| `payment.reversed` | The reorg-afterglow sweep confirmed the settlement is no longer on the canonical chain (fires alongside / instead of `payment.reorged`) — the authoritative rollback signal | **Recover** — if you already shipped goods or granted access, de-provision, cancel the order, claw back |
